...
分别报告规格要求但缺失或只部分实现的行为、diff 中未要求的 scope creep、看似实现但行为错误的需求；每项发现必须引用对应规格。返回 `verdict`、`blocking_findings`、`advisory_findings`、manifest digest 和 coverage；每个 finding 必须有稳定 ID、摘要和证据。coverage 与 findings summary 分字段返回，不施加摘要字数限制。
