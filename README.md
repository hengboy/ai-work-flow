...
- 将自定义技能（`run-matt-spec-to-completion`、`generate-ai-work-flow-agents`、`switch-ai-work-flow-env`、`project-code-navigation`）同步到 Codex、Claude Code 和 OpenCode 的全局 Skills 目录，并随受管 runtime 安装其执行依赖
...
设置 `XDG_CONFIG_HOME` 后，路径会变为 `$XDG_CONFIG_HOME/ai-work-flow/environments/default.json`。无 `.environment` 标记时直接使用默认环境；非默认环境由 `.environment` 指向 `environments/<name>.json`。环境文件会按角色、平台和字段覆盖默认环境，未提供的字段继承默认值；OpenCode 的 `options` 是例外，覆盖时整体替换，避免隐式保留旧选项。因此环境文件只需写差异字段，默认环境仍须保留全部受管理角色及三端完整配置。配置按角色和平台组织，例如：
...
非默认环境由 `.environment` 标记选择。`env use default` 通过删除该标记选择默认环境；无标记时已使用默认环境。该命令会先验证目标环境，再以单个事务重新生成所有已管理平台的 agents 并更新标记；任一步失败时会恢复原有 agents 和环境选择。
...
`$XDG_CONFIG_HOME/ai-work-flow/execution-runtime/execution-cli.mjs` 是执行状态的唯一 canonical 接口；未设置 `XDG_CONFIG_HOME` 时使用 `~/.config/ai-work-flow/execution-runtime/execution-cli.mjs`。所有执行模式，包括旧 `orchestrator` 计划，必须通过该 CLI 变更 Ticket 状态，不能直接写入 Checkpoint。
...
`claim` 使用跨进程排他锁，只允许一个执行者领取当前 Ticket。共享 runtime 的 `record-ticket` 从标准输入读取 JSON Handoff envelope，这是唯一的 canonical 状态转换输入。envelope 必须包含 `role_id`、`status`、`summary`、`artifacts`、`checks` 和类型化 `payload`，也可在 blocked 时包含 `error`；`payload` 必须是包含 `ticket_id`、`status`、`summary`、`commits`、`tests` 以及 blocked 时 `error` 的 Completion Result，且其 `status` 必须与 envelope 的 `status` 一致。裸 Completion Result 不是 runtime 接口。
...
Checkpoint 中的 worktree 只能是仓库内相对路径；绝对路径、`../` 遍历路径和其他仓库的 worktree 都会停止恢复，不进行迁移或兼容处理。
...
4. **评审与整合** — 完成 Standards + Spec 双轴评审；用户确认修复后直接整合，不再次评审，最后提交执行记录
...
- **Orchestrator 路由层**：`routing.md` + 9 角色 Agent 定义；代码评审优先使用已安装的 matt `$code-review` 流程，并始终由 AI Work Flow 的 **Review Standards** 与 **Review Spec** 子代理执行；未安装时回退至内置双轴提示词
