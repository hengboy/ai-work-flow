...
# 根据已有配置重新生成指定平台 agents
node scripts/install.mjs generate --platform codex,claude,opencode
...
`install` 是完整流程：同步 Skills、初始化配置和路由、安装运行时文件，然后生成 agents。`init`、`generate` 和 `validate` 适合后续单独维护配置或排查问题。
...
```sh
node scripts/install.mjs validate
node scripts/install.mjs generate
```
...
生成器只更新 AI Work Flow 管理的文件和配置片段，不覆盖其他全局 agent 或工具设置。新会话会读取重新生成后的 agents。
