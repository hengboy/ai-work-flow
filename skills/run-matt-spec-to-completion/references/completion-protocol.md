...
`execution-runtime/completion-result-schema.json` 是字段格式的权威。文本 `RESULT/COMMITS/TESTS` 不再是兼容入口。JSON Handoff envelope 必须带 `role_id`、`session_id` 和 `claim_id`，并与 payload 的 `status`、`summary`、`checks` 和可选 `error` 逐值一致；runtime 会将其与当前 Ticket 持久化的 claim identity 一次性核对。
