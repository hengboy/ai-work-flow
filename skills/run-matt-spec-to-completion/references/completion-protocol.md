...
Codex/Claude 和 OpenCode 分别注入自己的原生 `spawn`/`collect` 能力。没有原生能力时，`createUnsupportedAdapter(name)` 只为 Frontier 中按 ID 排序的首项返回结构化 blocked 结果，其余任务保持 pending；它绝不轮询或伪造任务。该适配器不自动重试：它没有 routing 所要求的暂态错误分类与旧子代理已停止证明。Orchestrator 仅可按 `routing.md` 在确认满足两项条件后，以新子会话安排重试。
