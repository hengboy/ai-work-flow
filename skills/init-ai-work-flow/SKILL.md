...
description: 为当前项目联合初始化根 MEMORY.md 与 .ai-work-flow/index/ 代码导航。首次在项目使用代码导航，或首次进入目录式 ReviewManifest 审查前使用。
---
...
5. 验证索引中的非计划路径真实存在，并检查 `MEMORY.md` 五个章节齐全。不得猜测入口、职责或边界。
6. `MEMORY.md` 是目录式 ReviewManifest 与提交绑定的 standards source，必须与索引一起提交。职责、模块边界、文件入口、路由或 API 变化时，必须在同一轮改动中同步维护 `MEMORY.md` 和对应索引。
