...
`completion-adapter.mjs` 是唯一解析终态协议的 module。`createNativeAdapter({ spawn, collect })` 按任务 ID 的确定顺序串行派发并收集 Frontier；第一个 blocked 结果或 spawn/collect 失败会立即停止，后续 Frontier 任务保持 pending 且不会派发。需要工作项时，原生任务从执行 worktree 的 `task.ref` 读取。
...
子代理名为 `Plan Task - {task_title}`，使用主代理的模型并加载 `implement` skill。它只编辑、测试和提交实现代码；Coordinator 委派 Full Stack Coder 在 main 更新本地任务复选框和 Checkpoint。
