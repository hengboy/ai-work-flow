...
import { beginReview, completeIntegration, completeReview, completeTask, createCheckpoint, markMerged, readCheckpoint, startTasks, writeCheckpoint } from "../lib/checkpoint.mjs";
import { createExecutionCoordinator } from "../lib/execution-coordinator.mjs";
import { materializeLocalPlan, writeExecutionPlan } from "../lib/execution-plan.mjs";
...
async function coordinatorFixture() {
  ...
  await writeFile(planPath, "# Migrate runtime\n");
  await writeFile(join(directory, "tasks", "01-contract.md"), "# Contract\n\n- [ ] Verify runtime contract\n");
  await git(root, "add", ".");
  await git(root, "commit", "-m", "fixture");
  const head = await git(root, "rev-parse", "HEAD");
  const executionPlan = await materializeLocalPlan({ mainWorktree: root, planPath, now: new Date("2026-07-23T12:00:00+08:00") });
  await writeExecutionPlan(root, executionPlan);
  let checkpoint = createCheckpoint({ executionPlan, baseline: head, branch: "plan/migrate-runtime", worktree: root, now: new Date("2026-07-23T12:00:00+08:00") });
  checkpoint = startTasks(checkpoint, ["01"], head);
  checkpoint = completeTask(checkpoint, "01", head);
...
async function pendingIntegrationFixture() {
  ...
  const head = await git(root, "rev-parse", "HEAD");
  let checkpoint = createCheckpoint({ executionPlan, baseline: head, branch: "plan/migrate-runtime", worktree: root });
  checkpoint = startTasks(checkpoint, ["01"], head);
  checkpoint = completeTask(checkpoint, "01", head);
  checkpoint = beginReview(checkpoint);
  checkpoint = completeReview(checkpoint, "approved");
  const executionWorktree = `${root}-execution`;
  await git(root, "worktree", "add", "-b", "plan/migrate-runtime", executionWorktree);
...
test("requires canonical planPath before either initialization or resume", async () => {
  const coordinator = createExecutionCoordinator();
  await assert.rejects(
    coordinator.run({ repository: "/not-used", branch: "plan/migrate-runtime" }),
    /canonical planPath is required to initialize or resume/,
  );
});
...
test("keeps merged cleanup recoverable when final record commit fails", async () => {
  ...
  await assert.rejects(
    failingCoordinator.completeMergedCleanup({ repository: root, mainWorktree: root, planId: "migrate-runtime", executionPlan, checkpoint: checkpointWithStash }),
  ...
  const resumed = await coordinator.resume({
    ...
    planPath: ".ai-work-flow/plans/migrate-runtime/plan.md",
    planId: "untrusted-plan-id",
...
test("reports an explicitly recorded but unavailable stash during merged cleanup", async () => {
  ...
  const coordinator = createExecutionCoordinator({ generateCommitMessage: async () => "chore: record execution" });
  await assert.rejects(
    coordinator.completeMergedCleanup({ repository: root, mainWorktree: root, planId: "migrate-runtime", executionPlan, checkpoint: checkpointWithStash }),
...
test("does not commit terminal records when their checkpoint fails integrity", async () => {
  ...
  await assert.rejects(
    coordinator.completeMergedCleanup({ repository: root, mainWorktree: root, planId: "migrate-runtime", executionPlan, checkpoint: invalidComplete }),
...
test("records a pre-merge stash operation before its stash reference can be persisted", async () => {
  ...
  await assert.rejects(
    failingCoordinator.integrate({ repository: root, worktree: executionWorktree, planId: "migrate-runtime", executionPlan, checkpoint }),
    /checkpoint storage unavailable/,
  );
  const recoverable = await readCheckpoint(root, "migrate-runtime");
  ...
  const coordinator = createExecutionCoordinator({ generateCommitMessage: async () => "chore: record execution" });
  const resumed = await coordinator.resume({ repository: root, branch: "plan/migrate-runtime", planPath: ".ai-work-flow/plans/migrate-runtime/plan.md", worktreePath: executionWorktree });
  assert.equal(resumed.status, "resumed");
  const completed = await coordinator.integrate({ repository: root, worktree: resumed.worktree, planId: "migrate-runtime", executionPlan, checkpoint: resumed.checkpoint });
...
test("does not stash, merge, or remove a worktree when integration integrity is invalid", async () => {
  ...
  const checkpointFile = join(root, ".ai-work-flow", "plans", "migrate-runtime", "checkpoint.json");
  const before = await readFile(checkpointFile, "utf8");
  const head = await git(root, "rev-parse", "HEAD");
  await writeFile(join(root, "unrelated.txt"), "preserve this change\n");

  await assert.rejects(
    createExecutionCoordinator().integrate({ repository: root, worktree: executionWorktree, planId: "migrate-runtime", executionPlan, checkpoint: invalid }),
    /Checkpoint integrity failed/,
  );
...
test("keeps undispatched tasks pending after the first serial blocked result", async () => {
  ...
  await writeFile(join(root, ".ai-work-flow", "plans", "migrate-runtime", "tasks", "02-follow-up.md"), "# Follow up\n");
  const planPath = join(root, ".ai-work-flow", "plans", "migrate-runtime", "plan.md");
  const twoTaskPlan = await materializeLocalPlan({ mainWorktree: root, planPath });
  await writeExecutionPlan(root, twoTaskPlan);
  const head = await git(root, "rev-parse", "HEAD");
  const checkpoint = createCheckpoint({ executionPlan: twoTaskPlan, baseline: head, branch: "plan/migrate-runtime", worktree: executionWorktree });
  await writeCheckpoint(root, "migrate-runtime", checkpoint);
  const dispatched = [];
  const coordinator = createExecutionCoordinator({
    adapter: {
      async executeFrontier({ tasks }) {
        ...
        return [{ task_id: "01", status: "blocked", commits: [], tests: [], summary: "blocked", error: "stop" }];
      },
    },
  });

  const result = await coordinator.executeFrontier({ worktree: executionWorktree, mainWorktree: root, planId: "migrate-runtime", executionPlan: twoTaskPlan, checkpoint });
  assert.deepEqual(dispatched, ["01"]);
  assert.equal(result.checkpoint.tasks.find((task) => task.id === "01").status, "blocked");
  assert.equal(result.checkpoint.tasks.find((task) => task.id === "02").status, "pending");
});

test("does not redispatch an in-progress task on recovery", async () => {
  const { root, executionPlan, executionWorktree } = await pendingIntegrationFixture();
  const head = await git(root, "rev-parse", "HEAD");
  const checkpoint = startTasks(createCheckpoint({ executionPlan, baseline: head, branch: "plan/migrate-runtime", worktree: executionWorktree }), ["01"], head);
  await writeCheckpoint(root, "migrate-runtime", checkpoint);
  let dispatched = false;
  const coordinator = createExecutionCoordinator({ adapter: { async executeFrontier() { dispatched = true; return []; } } });

  const result = await coordinator.executeFrontier({ worktree: executionWorktree, mainWorktree: root, planId: "migrate-runtime", executionPlan, checkpoint });
  assert.equal(result.status, "blocked");
  assert.equal(dispatched, false);
  assert.equal((await readCheckpoint(root, "migrate-runtime")).tasks[0].status, "in_progress");
});
...
test("recovers a pre-merge restoration after its restored checkpoint write fails", async () => {
  ...
  const resumed = await coordinator.resume({ repository: root, branch: "plan/migrate-runtime", planPath: ".ai-work-flow/plans/migrate-runtime/plan.md", worktreePath: executionWorktree });
  await assert.rejects(
    coordinator.integrate({ repository: root, worktree: resumed.worktree, planId: "migrate-runtime", executionPlan, checkpoint: resumed.checkpoint }),
...
test("does not mutate an invalid checkpoint while attempting a relocated resume", async () => {
  ...
  await assert.rejects(
    coordinator.resume({ repository: root, branch: "plan/migrate-runtime", planPath: ".ai-work-flow/plans/migrate-runtime/plan.md", worktreePath: join(root, "recreated-worktree") }),
    /Checkpoint integrity failed/,
  );
...
test("rejects complete checkpoints with pending work before terminal handling", async () => {
  ...
  invalidComplete.tasks[0] = { id: "01", status: "pending" };
  const checkpointFile = join(root, ".ai-work-flow", "plans", "migrate-runtime", "checkpoint.json");
  ...
  await assert.rejects(
    coordinator.resume({ repository: root, branch: "plan/migrate-runtime", planPath: ".ai-work-flow/plans/migrate-runtime/plan.md", worktreePath: join(root, "unused-worktree") }),
