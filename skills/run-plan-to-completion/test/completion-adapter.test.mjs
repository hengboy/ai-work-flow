...
test("dispatches and collects frontier tasks serially in task ID order", async () => {
  ...
  assert.deepEqual(results.map((result) => result.task_id), ["01", "02"]);
});
...
test("does not retry an unclassified native dispatch failure", async () => {
  ...
  const [result] = await adapter.executeFrontier({ tasks: [{ id: "01" }], worktree: "/tmp/execution" });
...
test("stops dispatching after a blocked completion", async () => {
  ...
  const results = await adapter.executeFrontier({ tasks: [{ id: "01" }, { id: "02" }], worktree: "/tmp/execution" });

  assert.deepEqual(dispatched, ["01"]);
  assert.deepEqual(results.map((result) => result.task_id), ["01"]);
...
test("unsupported adapters only block the first serial task", async () => {
  ...
  const results = await adapter.executeFrontier({ tasks: [{ id: "02" }, { id: "01" }], worktree: "/tmp/execution" });
