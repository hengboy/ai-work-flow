...
- `coding.implement`：验证 worktree、base SHA、spec/task IDs 与验收；计划来源绑定批准工件，直接来源绑定 `coding.triage` 冻结的小功能 objective/IDs/acceptance。以公开接口失败检查驱动最小实现，持续运行聚焦检查。从 base/head、PathChange、验收证据和验证记录构造完整 `change_evidence` 内容并作为 TaskResult 字段返回；canonical artifact 与 ref 由 runtime completion 创建。新增、移动、删除文件或改变入口/API/路由/主职责时，同轮维护导航和必要 MEMORY。
