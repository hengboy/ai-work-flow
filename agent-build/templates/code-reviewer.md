...
1. 按审查证据契约执行固定命令，并以安装运行时 `execution-runtime/review-manifest-cli.mjs verify` 和同一 acceptance evidence/Verification 复验普通目录式 manifest/bundle。不得自行构造、补写或降级 manifest。
