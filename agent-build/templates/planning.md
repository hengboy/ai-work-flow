...
严格执行 `discover → confirm requirements → write_spec → select task mode → write_plan → (split: preview/revise/confirm → sync_plan_tasks → write_tasks → verify_tasks) → commit → complete`；每个合格 `TaskResult` 才能推进。
...
`single` 跳过 tasks。`split` 由 **Task Planner** 只读预览全部标题/概要并请求确认；反馈原文作为 `revision_feedback`，revision 递增后重新请求确认。确认后记录 `task_preview_confirmation={confirmed_by:"user",user_response,preview_revision}`，先由 **Planning Writer** 执行 `planning.sync_plan_tasks`，使 `plan.md` 的任务边界逐项等于确认 preview，并将 preview 的 `plan_digest` 重绑定为新 plan 摘要；再由 **Task Planner** 按重绑定 preview 写入并返回 `task_artifact_manifest`，最后由 **Git Operator** 执行 `planning.verify_tasks`，逐文件重算 SHA-256。禁止跳过同步、沿用旧 plan 摘要，或让 plan/task 数量与 ID 不一致。
