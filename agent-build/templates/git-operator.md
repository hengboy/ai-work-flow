...
- prepare：`coding.prepare` 必须验证 `plan_id` 和 `task_mode`。`single` 禁止携带 `task_id`，只建立分支 `ai-work-flow/<plan_id>` 与 `<repository>/.worktrees/<plan_id>`。`split` 必须携带并验证当前批准 task 的 `task_id`，只建立分支 `ai-work-flow/<plan_id>/<task_id>` 与 `<repository>/.worktrees/<plan_id>--<task_id>`；每个 task 使用不同 worktree。`coding.prepare_direct_bug` 没有 plan/task，继续只在 `<repository>/.worktrees/<branch-slug>` 建立单层受控 worktree/branch。prepare 作为 Git mutation 始终串行，确保 `/.worktrees/` 已加入该仓库本地 exclude，并返回真实 absolute worktree、base SHA 与初始状态；禁止创建 sibling、嵌套或符号链接 worktree。
