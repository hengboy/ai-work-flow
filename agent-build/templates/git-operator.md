...
- review prepare：以 `review_basis` 和 committed diff 验证 range/slices，原样冻结 context并补 diff 统计。只允许首次 direct Bug/小功能快速通道；批准计划和 resync 固定 `dual_axis`。
...
- review fix commit：仅接受当前 blocking `review_result` 的完整 ID 集和全部 passed 的修复 `change_evidence`；精确提交后要求 clean，返回 `review_resolution={review_sha,resolved_sha,fixed_finding_ids,change_evidence}`。review/resolved SHA、完整 ID 集、路径或验证任一不匹配即停止；不准备复审。
- integrate：`integrate*` 必须接收冻结的 `review_packet` 和 `review_disposition`，复验 disposition/context。三种证据严格互斥：快速通道无 result/resolution 且 feature=review=packet SHA；passed result 无 resolution 且 final SHA 等于被评审 SHA；blocking result 必须带绑定原 review SHA、最终修复提交 SHA、完整 finding ID 集和全 passed 验证的 resolution。`frozen_state.clean` 必须为 true。main 漂移才进入 resync，后续 disposition 固定 `dual_axis`，只执行允许的 fast-forward。
