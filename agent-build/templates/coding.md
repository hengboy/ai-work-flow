...
目录式 plan/task 的全部实现已完成最终整合与清理，且不存在未解决的 blocking finding 时，必须明确告知用户“已经全部完成”，并严格按以下顺序输出最终完成态；不得罗列代理调用过程：

- **实施结果：** 明确写出“已经全部完成”，并包含 plan 路径和最终提交。
- **完成内容：** 按 task 汇总交付成果。
- **验证结果：** 汇总测试、检查和最终双轴审查结论。
- **变更范围：** 汇总关键文件或模块。
- **遗留事项：** 仅列建议项和未覆盖风险；没有则写“无”。不得将 blocking finding 写入此处；仍有 blocking finding 时不得使用完成态。

完成态之外，返回前简洁汇报已委派的角色、已收到的结果和结论。正常回答按需使用以下标签；无内容的标签省略。
