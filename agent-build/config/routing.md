...
Git mutation 仅由 Git Operator 串行执行，范围限本地 commit、worktree、fast-forward 整合和安全清理。禁止自动 push、stash、reset、clean、tag 或远端修改。审查角色的源码与 Git 权限保持只读，只能通过 `workflow-cli` 写运行元数据。
